...
class LSTMClassification(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_top3_accuracy', self.top3_accuracy(y_hat, y))
...
class MatchboxNet(nn.Module):
    ...
    **Arguments**
    ...
    def __init__(self, B, R, C, NUM_CLASSES=25, kernel_sizes=None):
        ...
        if not kernel_sizes:
            kernel_sizes = [k*2+11 for k in range(1,6+1)] # incrementing kernel size by 2 starting at 13
    ...
    def forward(self, x):
        ...
        # prologue block
        # print('input: ', x.shape)
        x = self.prologue_conv1(x) # (64, 141) > (128, 66)
        x = self.prologue_bnorm1(x)
        x = F.relu(x)
        # print('prologue: ', x.shape)

        # intermediate blocks
        for layer in self.blocks:
            x = layer(x) # (128, 66) > (C, 66)
        # print('intermediate: ', x.shape)

        # epilogue blocks
        x = self.epilogue_conv1(x) # (C, 66) > (128, 10)
        x = self.epilogue_bnorm1(x)
        # print('epilogue1: ', x.shape)

        x = self.epilogue_conv2(x) # (128, 10) > (128, 10)
